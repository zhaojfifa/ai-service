...
function applyDefaults() {
  ...
  if (layoutStructure) {
    layoutStructure.textContent = '点击“构建版式预览”按钮以查看布局结构概览。';
  }
  ...
  if (aiPreview) {
    aiPreview.classList.remove('complete');
  }
  if (aiSpinner) {
    aiSpinner.classList.remove('hidden');
  }
  if (aiPreviewMessage) {
    aiPreviewMessage.textContent = '正在调用 Glibatree Art Designer 生成海报，请稍候…';
  }
  if (posterVisual) {
    posterVisual.classList.add('hidden');
  }
  if (promptGroup) {
...
function syncEmailFields(data, emailText) {
  ...
  const agentLabel = data.agent_name ? `（${data.agent_name}）` : '';
  const productLabel = data.product_name || '产品';
  emailSubject.value = `${brandLabel}${agentLabel} ${productLabel} 市场推广海报`;
  if (!emailRecipient.value) {
    emailRecipient.value = defaultEmailRecipient;
  }
  if (emailText) {
...
function updatePreview(options = {}) {
  ...
  const layoutData = {
    ...
    brand_name: data.brand_name || '品牌名',
    agent_name: data.agent_name || '代理名 / 分销名',
  ...
  previewBrandLogo.alt = data.brand_name
    ? `${data.brand_name} 品牌 Logo`
    : '品牌 Logo 预览';
  if (previewBrandName) {
    previewBrandName.textContent = layoutData.brand_name;
  }
  previewAgentName.textContent = layoutData.agent_name.toUpperCase();
...
async function generatePoster(options = {}) {
  ...
  try {
    ...
    if (aiPreview) {
      aiPreview.classList.add('complete');
    }
    if (aiPreviewMessage) {
      aiPreviewMessage.textContent = 'AI 生成完成，以下为最新海报预览。';
    }
    if (posterVisual) {
    ...
    if (aiPreviewMessage) {
      aiPreviewMessage.textContent = error.message || '生成海报时发生错误。';
    }
    if (regenerateButton) {
      regenerateButton.classList.remove('hidden');
    }
