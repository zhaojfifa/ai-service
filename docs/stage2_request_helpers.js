(function initStage2RequestHelpers(globalScope) {
  ...
  function buildStage2PreflightDiagnostics({
    ...
    return {
      ...
      raw_bottom_mode: rawBottomMode ?? null,
      canonical_bottom_mode: canonicalBottomMode ?? formSignatures?.bottom?.bottom_mode ?? null,
      payload_bottom_mode: payloadBottomMode ?? payload?.bottom_mode ?? null,
      current_bottom_mode: formSignatures?.bottom?.bottom_mode || null,
      health_status: healthStatus ?? 'unknown',
      template_posters_status: templatePostersStatus ?? 'unknown',
