...
## 8. 远程探针结果（push 后）/ Remote probe after push

- Push: `143a7f0` → origin `trial/poster2-cuistance-v1-operator-trial`（已推送）。
- `https://ai-service-leob.onrender.com/cuistance_trial.html` → **HTTP 404**（push + 等待后仍 404）。
- 对照：同主机 `/ops_campaign.html` → **200**，`/health` → **200**（静态服务正常）。
- **解读：** leob 当前**未提供** trial 分支的新页面 —— 要么部署尚未刷新，要么 leob 指向的并非
  `trial/poster2-cuistance-v1-operator-trial` 分支（其服务的内容仍含旧 ops_campaign）。这是**部署目标/刷新**事项
  （与上一任务的 deploy Owner-Decision-Needed 同源），非页面或后端缺陷。
- **远程可见前置条件：** 将 trial 服务部署刷新到提交 `143a7f0`（或把验证服务指向 trial 分支）。刷新后
  `/cuistance_trial.html` 应可公开打开（静态页不受 OPS 门控；其 API 调用才需 `Se connecter`）。
