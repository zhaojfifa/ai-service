...
## 2026-03-31 — Task-2b annotation lane truth alignment
...
**Problem**
- Task-2 shell resize was already live
- `product_region` / primary / secondary slots had been expanded
- but annotation item bounds were still using the old fixed right-lane positions derived from template callout spec
- that left runtime with a contract mismatch:
  - product shell geometry was new
  - annotation shell / label evidence was still partially old

**Decision**
- Chose one truth for the annotation lane:
  - explicit external right-side lane declared by runtime contract
- Kept ownership unchanged:
  - `annotation_owner_slot = product_primary_slot`
  - `secondary_slot_annotation_ownership = false`

**What changed**
- resolver and renderer now both consume the same product-annotation lane derived from final product geometry
- old fixed `x=784 / w=144` label positions are no longer used in runtime for `product_anchor_callouts`
- `product_annotation_shell_layer.bounds` is now:
  - `{x:848, y:224, w:128, h:252}`
- annotation slot 1 evidence is now:
  - `anchor_x = 812`
  - `anchor_y = 260`
...
- the shell had already widened to `product_region.w = 376`
- but annotation items still behaved as if the old fixed template lane was the source of truth
- that meant contract/evidence and runtime drawing could drift apart
...
**Fresh runtime proof**
- local API path: `POST /api/v2/generate-poster`
- trace: `2b4e8a7e-7e29-4df7-b2a8-008fe0917b2b`
- `degraded = false`
- `structure_complete = true`
- `deliverable = true`
- `product_geometry_mode = primary_secondary_dual_v3`
