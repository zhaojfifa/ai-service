# PR-5 — Text Capacity / Label Bounds / Clamp / Connector Tuning Status v1
...
PR-5 tunes product-owned annotation readability on top of the PR-4 geometry baseline.

In scope:
- Increase annotation label box height 60 → 76 for active slots 1-3
- Increase line clamp 2 → 3 for product_anchor_callouts (CSS + Pillow renderer)
- Expand `_PRODUCT_TEXT_SHELL_H` 260 → 276 (tracks slot_3 bottom at y=416+h=76=492; 492−216=276)
...
- Slot 4 label_box (anchor_y=550) — excluded by max_items=3; kept at h=60 to preserve region_bottom for HTML repositioning path
...
24px inter-slot gap is clear visual separation. The old 40px gap was unused vertical slack.
...
Both within `product_region` bottom (188+540=728) ✓
...
**3-line clamp**: w=144 at font_size=15 (Noto Sans SC) fits ~16-18 CJK chars per line. With 2-line clamp, max visible content ≈ 32-36 chars; 3-line clamp raises that to ~48-54 chars. Directly reduces truncation for medium-length feature callout copy.

**Label box h 60→76**: Vertical padding at 8px top + 8px bottom = 16px. 3 lines at line_height=1.3 × font_size=15 ≈ 58.5px rendered height + 16px padding = 74.5px → rounded to 76px. Ensures 3 lines render without overflow within the box bounds.

**Gap reduction 40→24px**: The 40px inter-slot gap was larger than the padding inside the box. Reducing to 24px is a clear visual break while recovering 16px per slot for content.

**Char budget**: Raised to match the physical capacity of the label box (w=144, 3 lines at font_size=15). The char_budget field is a contract/diagnostic signal — actual rendering is controlled by CSS line-clamp and Pillow max_lines. Raising the budget closes the gap between the declared budget and actual box capacity.
...
2. `app/templates_html/template_dual_v2.css`
   - Added `.product-annotation-mode-product_anchor_callouts .feature-callout { -webkit-line-clamp: 3; line-clamp: 3; min-height: 76px; }` after the feature-mode-4 rule block
...
5. `tests/poster2/test_pipeline.py`
   - Updated 4 assertions: `h==260` → `h==276`
   - Renamed `test_text_shell_bounds_unaffected` → `test_text_shell_bounds_after_prc`
   - Added `TestProductTextCapacityPRC` class (12 tests)
...
- CSS: `-webkit-line-clamp: 3` for product_anchor_callouts mode only
...
### Truncation improvement evidence

| Callout length | Before (2-line, budget=28) | After (3-line, budget=32) |
|---|---|---|
| ≤ 28 chars | fully visible | fully visible |
| 29–32 chars | **truncated** (budget) | fully visible |
| 33–48 chars | **truncated** (budget + clamp) | clamp may trim last chars |
| > 48 chars | truncated | truncated (3-line physical limit) |

Net: ~14% more chars survive without truncation via budget; ~50% more line capacity via clamp.

---

## 7. Fresh Runtime Proof

Full pipeline run with three annotation texts via fake renderer (real pipeline path, real contract resolution).

**Input texts:**
- slot_1: `"超薄防水面料，耐磨耐用"` (len=11)
- slot_2: `"三层复合内胆，轻量保温高效锁热防护"` (len=17)
- slot_3: `"人体工学肩带，长途背负舒适减压，可调节宽度适配多种体型"` (len=27)

**Manifest key fields:**

```
product_text_shell_h : 276
line_clamp           : 3
char_budget          : 32
...
product_annotation_slot_1:
  label_bounds.h     : 76
  requested_text     : "超薄防水面料，耐磨耐用"
  sanitized_text     : "超薄防水面料，耐磨耐用"
  rendered_excerpt   : "超薄防水面料，耐磨耐用"
  truncation_applied : False

product_annotation_slot_2:
  label_bounds.h     : 76
  requested_text     : "三层复合内胆，轻量保温高效锁热防护"
  sanitized_text     : "三层复合内胆，轻量保温高效锁热防护"
  rendered_excerpt   : "三层复合内胆，轻量保温高效锁热防护"
  truncation_applied : False

product_annotation_slot_3:
  label_bounds.h     : 76
  requested_text     : "人体工学肩带，长途背负舒适减压，可调节宽度适配多种体型"
  sanitized_text     : "人体工学肩带，长途背负舒适减压，可调节宽度适配多种体型"
  rendered_excerpt   : "人体工学肩带，长途背负舒适减压，可调节宽度适配多种体型"
  truncation_applied : False
```

**Budget improvement boundary (old budget=28 vs new budget=32):**

```
len=27: @budget=28 trunc=False  @budget=32 trunc=False
len=29: @budget=28 trunc=True   @budget=32 trunc=False  <- IMPROVED
len=31: @budget=28 trunc=True   @budget=32 trunc=False  <- IMPROVED
len=33: @budget=28 trunc=True   @budget=32 trunc=True
```

Texts of len 29-32 that were truncated in PR-4 baseline now survive without truncation.
...
python3 -m pytest -q tests/poster2/test_contracts.py
→ 17 passed
```
...
- JSON slot 4: h=60, max_lines=2 (excluded, unchanged) ✓
- CSS: `.product-annotation-mode-product_anchor_callouts .feature-callout { -webkit-line-clamp: 3; min-height: 76px }` ✓
...
- HTML repositioning path (`_resolve_feature_callout_map`) recomputes y positions from `region_top/region_bottom` using `box_h` from `feature_policy` — for product_anchor_callouts with `box_h=72`, slots are repositioned to [288, 360, 432] rather than the template-spec fixed positions. This is pre-existing behavior; PR-5 does not change it.
- Slot 4 kept at h=60 intentionally to preserve `region_bottom=576` for the HTML path. If slot 4 is ever made live (max_items raised to 4), this constraint must be re-evaluated.
...
- `feature_region` continues to be delegated diagnostic when product annotation is active — unchanged by PR-5.
